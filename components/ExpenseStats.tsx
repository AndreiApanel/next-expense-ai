...
const ExpenseStatsContent = ({
  ...
  validDays: number;
  bestExpense: number | undefined;
  worstExpense: number | undefined;
}) => {
  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl border border-gray-100/50 dark:border-gray-700/50 hover:shadow-2xl">
      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-emerald-500 from-blue-600 via-blue-500  rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div className="bg-green-50/80 dark:bg-green-900/20 backdrop-blur-sm p-3 sm:p-4 rounded-xl border-l-4 border-l-green-500 hover:bg-green-50 dark:hover:bg-green-900/30">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-green-100 dark:bg-green-800 rounded-xl flex items-center justify-center flex-shrink-0">
                <span className="text-sm leading-none text-green-600 dark:text-green-300 font-bold">↓</span>
              </div>
              <div className="flex-1">
...
const ExpenseStats = async () => {
  ...
  try {
    ...
    [userRecordResult, rangeResult] = await Promise.all([getUserRecord(), getBestWorstExpense()]);
  } catch (error) {
